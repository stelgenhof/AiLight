            ...
            function (e) {
                if (e.data.startsWith('p-')) {
                    ...
                    if (p === 100 && !hS) {
                        ...
                        f.innerHTML =
                            'Completed successfully! Please wait for your AiLight Smart Light to be restarted.'
...
function reset() {
    let response = window.confirm(
        'You are about to reset your AiLight Smart Light to the factory defaults!\n Are you' +
            ' sure you' +
            ' want to reset?'
    )
    if (response === false) {
    ...
    for (let i = 0; i < inputs.length; i++) {
        ...
        if (id === 'wifi_psk') {
            if (
                ...
                (inputs[i].value.length > 63 || inputs[i].value.length < 8)
            ) {
        ...
        if (id === 'api_key') {
            if (
                inputs[i].value &&
                inputs[i].value.length > 0 &&
                (inputs[i].value.length > 32 || inputs[i].value.length < 8)
            ) {
